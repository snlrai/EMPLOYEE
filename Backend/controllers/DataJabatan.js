...
export const getJobPositionData = async (req, res) => {
  try {
    ...
    if (req.hak_akses === "admin") {
      ...
      if (req.userId !== EmployeeRole.userId)
      ...
      await EmployeeRole.update(
        ...
        },
        {
          where: {
            [Op.and]: [
              { id_jabatan: jobPosition.id_jabatan },
              { userId: req.userId },
            ],
          },
        }
...
export const createJobPositionData = async (req, res) => {
  ...
  try {
    ...
    res
      .status(201)
      .json({ success: true, message: "Job Position Data Successfully Saved" });
  } catch (error) {
...
export const deleteJobPositionData = async (req, res) => {
  try {
    ...
    if (req.hak_akses === "admin") {
      ...
      if (req.userId !== jobPosition.userId)
        return res.status(403).json({ msg: "Unauthorized access" });
      await jobPosition.destroy({
        where: {
          [Op.and]: [
